...
compatibility: Requires this dsh plugin's You.com web provider (`web_search` is You.com; set `YDC_API_KEY` for keyed access; without a key search falls back to the keyless MCP profile).
license: MIT
metadata:
...
Use `web_search` to discover web sources and `web_fetch` to read specific URLs. Search finds candidate sources; reading extracts reliable evidence.
...
1. **Search broadly**: `web_search` to find relevant pages. Read snippets to identify which pages have the data you need.
2. **Read content**: Call `web_fetch(url)` on the most promising URLs, one URL per call (loop over the 1-3 most promising). Snippets alone are unreliable — you must read the actual page to get exact values. Always read at least one page before answering.
3. **If incomplete**: refine the query and search again. If the question names a source (e.g., "according to the CDC"), pin its domain inline in the query: `web_search(query="... site:cdc.gov")`.
...
- Snippets never count as reading. For a single focused fact, the snippets may suffice; for anything complex, read the page with `web_fetch` — do not treat snippet output as full reading.
